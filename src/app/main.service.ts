...
export class MainService {
  ...
  public uploadData(inputFile: string | ArrayBuffer | null): void {
    if(inputFile != null) {
      ...
      //generate objects
      const data = utils.sheet_to_json<any>(workSheet, {header: 1});
  ...
  private toAmbulatorioTableIterface(riga: RigaInterface): AmbulatorioTableInterface {
    let model: AmbulatorioTableInterface = {
      ...
      data_inserimento: riga.data_inserimento,
  ...
  private toRigaInterface(row: string[], rowNumber: number): RigaInterface {
    let riga: RigaInterface = {
      row_number: rowNumber,
      discriminante: row[0],
      data_inserimento: row[1],
      email: row[2],
      nome: row[3],
      cognome: row[4],
      luogo_nascita: row[5],
